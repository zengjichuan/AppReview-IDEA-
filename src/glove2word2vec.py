...
def glove2word2vec(glove_input_file, word2vec_output_file):
    ...
    if sys.version_info < (3,):
        ...
        with smart_open(word2vec_output_file, 'w') as fout:
            print(num_lines, num_dims)
            fout.write(str(num_lines) + " " + str(num_dims) + "\n")
            with codecs.open(glove_input_file, 'r') as fin:
                for idx, line in enumerate(fin):
                    try:
                        print("Line %d succeeds." % idx)
                        fout.write(line)
                    except UnicodeDecodeError as e:
                        print(e)
                        print(line)
                        fout.write(re.sub(r"[^0-9a-z,.?!\-' ]", '', line))
        return num_lines, num_dims
