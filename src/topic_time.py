...
apps = [("/home/jczeng/workspace/AppReview/data/raw/mysql/Specific_apps_iTunes/Facebook_it/Facebook_it_clean_data.txt", "facebook"),
        ("/home/jczeng/workspace/AppReview/data/raw/mysql/Specific_apps_iTunes/YouTube_it/YouTube_it_clean_data.txt", "youtube"),
        ("/home/jczeng/workspace/AppReview/data/raw/mysql/Specific_apps_iTunes/WhatsApp_Messenger_it/WhatsApp_Messenger_it_clean_data.txt", "whatapp"),]
new_apps = [
    ("/home/jczeng/Dropbox/ASE'17/dataset/youtube/total_info.txt", "youtube"),
    ("/home/jczeng/Dropbox/ASE'17/dataset/tunein-radio-stream-free-music/total_info.txt", "tune"),
    ("/home/jczeng/Dropbox/ASE'17/dataset/photo-grid-collage-maker-fx-editor/total_info.txt", "photo"),
    ("/home/jczeng/Dropbox/ASE'17/dataset/noaa-radar-pro-severe-weather/total_info.txt", "radar")
]

google_apps = [
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/clean_master/total_info.txt", "cleanmaster"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/evernote/total_info.txt", "evernote"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/swiftkey_keyboard/total_info.txt", "swiftkey"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/battery_doctor/total_info.txt", "battery"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/viber/total_info.txt", "viber"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/line/total_info.txt", "line"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/ebay/total_info.txt", "ebay"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/booking_hotel_reservations/total_info.txt", "booking"),
    # ("/home/jczeng/Dropbox/ASE'17/dataset/android/spotify_music/total_info.txt", "spotify"),
    ("/home/jczeng/Dropbox/ASE'17/dataset/android/battery_doctor/total_info.txt", "battery")
]

validate_files = {
    "youtube": "/home/jczeng/Dropbox/ASE'17/dataset/changelog_youtube.txt",
    "tune": "/home/jczeng/Dropbox/ASE'17/dataset/changelog_tune.txt",
    "photo": "/home/jczeng/Dropbox/ASE'17/dataset/changelog_photo.txt",
    "radar": "/home/jczeng/Dropbox/ASE'17/dataset/changelog_radar.txt",
...
# extract reviews with time stamp
def extract_time_review():
    timed_reviews = {}
    num_docs = 0
    num_words = 0
    for app, apk in apps:
        ...
        for line in lines:
            ...
            date = terms[4]
            rate = float(terms[5]) if re.match(r'\d*\.?\d+', terms[5]) else 2.0
            version = terms[6]
...
def extract_reviews(reviews_p=None):
    if reviews_p == None:
        timed_reviews = {}
    else:
        timed_reviews = reviews_p
    num_docs = 0
    num_words = 0
    for app, apk in google_apps:
        timed_reviews[apk] = []
        with open(app) as fin:
            lines = fin.readlines()
        for line in lines:
            ...
            review = list(build_phrase(review_p))
            review = [list(replace_digit(s)) for s in review]
            # date = datetime.strptime(terms[3], "%b %d, %Y").strftime('%Y%m%d')
            date = datetime.strptime(terms[2], "%Y-%m-%d").strftime('%Y%m%d')
...
def update_phrase():
    ...
    # update
    bigram.save("../model/phrase/bigram.model")
    trigram.save("../model/phrase/trigram.model")

# update w2v model
def update_w2v():
    # load wv_model
    wv_model = Word2Vec.load("../model/appreviews_word2vec.model")
    reviews = []
    for app, apk in new_apps:
        with open(app) as fin:
            lines = fin.readlines()
        for line in lines:
            line = line.strip()
            ...
            review_p = trigram[bigram[review_p]]
            reviews.append(review_p)
    # wv_model.build_vocab(reviews)
    wv_model.train(reviews)
    wv_model.save("../model/wv/word2vec_app.model")

def load_phrase():
    global bigram
    global trigram
    bigram = Phrases.load(os.path.join("..", "model", "phrase", "bigram.model"))
    trigram = Phrases.load(os.path.join("..", "model", "phrase", "trigram.model"))
...
def build_OLDA_input_time(time_interval=7, start_time='20151101', end_time='20160301'):
    logging.info("from %s to %s, interval is %d days"%(start_time, end_time, time_interval))
    with open("../result/ldaresult/new_timed_reviews") as fin:
        timed_reviews = json.load(fin)
    stoplist = stopwords.words('english') + my_stoplst
    # pre assign the time slot, each time slot (start, end) corresponds to an index
    period = date_range_days(start_time, end_time)
    n = int(period) / time_interval + 1
    OLDA_input = {}

    for apk, reviews in timed_reviews.items():
        input = [[] for k in range(n)]  # init n [] time slot
        rate = [[] for k in range(n)]
        tag = []
        for review in reviews:
            review_data = review['date']
            # check review date is within the period
            if not datetime.strptime(start_time, '%Y%m%d') <= datetime.strptime(review_data, '%Y%m%d') <= datetime.strptime(end_time, '%Y%m%d'):
                continue
            i = int(date_range_days(start_time, review_data)) / time_interval
            input[i].append(review['review'])
            rate[i].append(review['rate'])
        ...
        for t_i, text_period in enumerate(input):
            tag.append((datetime.strptime(start_time, '%Y%m%d')+timedelta(days=t_i*time_interval)).strftime('%Y%m%d'))
            ...
            for k, text in enumerate(text_period):
                for i, j in dictionary.doc2bow(text):
                    ...
                    col.append(i)
                    value.append(j)
            input_X.append(sparse.coo_matrix((value, (row, col)), shape=(len(text_period), len(dictionary))))
        OLDA_input[apk] = (dictionary, input_X, input, rate, tag)      # input: raw input, with time and sent
        break   ############
    return OLDA_input

def build_OLDA_input_version():
    with open("../result/ldaresult/new_timed_reviews") as fin:
        timed_reviews = json.load(fin)
    stoplist = stopwords.words('english') + my_stoplst
...
def OLDA_train(OLDA_input):
    import time
    phrases = generate_labeling_candidates(OLDA_input)
    apk_phis = {}
    for apk, item in OLDA_input.items():
        dictionary, input_X, _, _1, _2 = item
        for tp in [1,2,3,4,5,6,7]:
            # fout = open("../result/ldaresult/%s_ntopic%s_winsize%s_train"%(apk, tp, 3), 'w')
            # fout_d = open("../result/ldaresult/%s_ntopic%s_winsize%s_divergence"%(apk, tp, 3), 'w')
            for i in range(5):
                olda_model = OLDA(n_topics=10, n_iter=1000, refresh=1000, window_size=tp, theta=0.5)
                olda_model.fit(input_X)
                # for ll in olda_model.loglikelihoods_train:
                #     fout.write("%f\t"%ll)
                # fout.write("\n")r
                apk_phis[apk] = olda_model.B
                ########################
                apk_jsds = topic_labeling({apk: item}, apk_phis, phrases, 0.5, 0.75, 0.0, save=False)
            #     jsds = np.reshape(np.array(apk_jsds[apk]), (-1, 10))
            #     jsds_avg = np.sum(jsds, 1)
            #     for j, js in enumerate(jsds_avg):
            #         if j % len(input_X) == 0:
            #             fout_d.write('\n')
            #         fout_d.write("%f\t"%js)
            # fout.close()
...
def OLDA_fit(OLDA_input, n_topics, win_size):
    ...
    for apk, item in OLDA_input.items():
        ...
        fout = open("../result/ldaresult/topic_words_%s_%s_%s"%(apk, n_topics, win_size), 'w')
...
def topic_labeling(OLDA_input, apk_phis, phrases, mu, lam, theta, save=True):
    logging.info("mu: %f, lam: %f, theta: %f"%(mu, lam, theta))
    apk_jsds = {}
    for apk, item in OLDA_input.items():
        ...
        if save:
            fout_labels = open("../result/ldaresult/topic_labels_%s"%apk, 'w')
            fout_emerging = open("../result/ldaresult/emerging_topic_labels_%s"%apk, 'w')
            fout_sents = open("../result/ldaresult/topic_sents_%s"%apk, "w")
            fout_emerging_sent = open("../result/ldaresult/emerging_topic_sents_%s" % apk, 'w')
            fout_topic_width = open("../result/ldaresult/topic_width_%s" % (apk), 'w')
        for t_i, phi in enumerate(phis):
            # need to count for each epoch
            # if t_i == 0:     # ignore first 6 epoch for memory efficient
            #     continue
            # label topic
            # logging.info("labeling topic at %s slice" % (t_i))
            ...
            # logging.info("detecting topic at %s slice" % (t_i))
            if save and t_i == 0:
...
def rank_topic_label(count, total_count, phi, label_ids, mu=0.2):
    ...
    #         # logging.info("label: %s"%label_id)
    #         score = 0
    #         # similarity of the topic dist of phi
    #         score += sim_topic_word(phi_i, label_id, count)
    #         # divergence of other topic dist
    #         diverg = 0
    #         for t_j, phi_j in enumerate(phi):
    #             if t_j == t_i:
    #                 continue
    #             diverg += sim_topic_word(phi_j, label_id, count)
    #         diverg /= (len(phi) - 1)
    #         score = score - mu * diverg
    #         label_scores[label_id] = score
    #     topic_label_scores.append(label_scores)
...
def topic_detect(rawinput_sents, dic, phi, last_phi, count, last_count, total_count, last_total_count, label_ids, sent_ids, sensi_label, sensi_sent, jsds, theta, mu, lam):
    ...
    #
    # for t_i, phi_i in enumerate(phi):
    #     if js_d[t_i] >= js_mean + 1.25*js_std:
    #         # emerging topic found
    #         logging.info("emerging topic found at %s slice!!"%(t_i))
    #         # labeling
    #         label_scores = np.dot(phi_i, np.transpose(c_label_m)) - theta * np.dot(last_phi[t_i], np.transpose(c_last_label_m)) + lam * sensi_label
    #         sent_scores = np.dot(phi_i, np.transpose(sent_count)) - phi_logphi - theta * (np.dot(last_phi[t_i], np.transpose(sent_count)) - phi_logphi_last) + lam * sensi_sent
    #         emerging_label_scores.append(label_scores)
    #         emerging_sent_scores.append(sent_scores)
    #     else:
    #         emerging_label_scores.append([])
    #         emerging_sent_scores.append([])
    return emerging_label_scores_rst, emerging_sent_scores_rst
...
def validation(logfile, label_phrases, label_sents, emerge_phrases, emerge_sents):
    ...
    with open("../result/ldaresult/params.txt", "a") as fout:
...
if __name__ == '__main__':
    ...
    # with open("../result/ldaresult/new_timed_reviews") as fin:
    #     timed_reviews = json.load(fin)
    # new_timed_reviews = extract_reviews(timed_reviews)
    # # timed_reviews = extract_time_review()
    # save_rst("../result/ldaresult/new_timed_reviews", new_timed_reviews)

    # OLDA_input = build_OLDA_input()
    OLDA_input = build_OLDA_input_version()
    start_t = time.time()
    apk_phis = OLDA_fit(OLDA_input, 10, 3)
    phrases = generate_labeling_candidates(OLDA_input)
    topic_labeling(OLDA_input, apk_phis, phrases, 1.0, 0.75, 0.0, save=True)
    print("%s seconds"%(time.time() - start_t))
    # OLDA_train(OLDA_input)
    # for w in range(1, 7):
    #     for i in range(3):
    #         apk_phis = OLDA_fit(OLDA_input, 12, w)
    #         save_obj("../model/ldamodel/OLDA_youtube_%d_win_%d.model"%(12, w), apk_phis)
    #         # apk_phis = load_obj("../model/ldamodel/OLDA_youtube_12.model")
    #         phrases = generate_labeling_candidates(OLDA_input)
    #         # # save_rst("../result/ldaresult/phrase_candidates", phrases)
    #         # mu = [0.25, 0.5, 0.75]   # 0.5
    #         # lam = [0.25, 0.5, 0.75]  # 0.5
    #         # theta = [1.0]
    #         # for m in mu:
    #         #     for l in lam:
    #         #         for t in theta:
    #         #             topic_labeling(OLDA_input, apk_phis, phrases, m, 0.35, l, t)
    #         topic_labeling(OLDA_input, apk_phis, phrases, 0.5, 0.75, 1.0)
